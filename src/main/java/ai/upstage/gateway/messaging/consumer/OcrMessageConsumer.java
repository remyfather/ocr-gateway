package ai.upstage.gateway.messaging.consumer;

import ai.upstage.gateway.messaging.config.RabbitMQConfig;
...
public class OcrMessageConsumer {
    ...
    private void processSingleDocument(DocumentClassificationResult doc, String accidentNumber,
        ...
        try {
            log.info("문서 처리 중 - 파일ID: {}, 문서타입: {}, 신뢰도: {}",
                    doc.getFileId(),
                    doc.getDetectedDocumentType().getDescription(),
            ...
            log.info("문서 처리 완료 - 파일ID: {}", doc.getFileId());

        } catch (Exception e) {
            log.error("문서 처리 실패 - 파일ID: {}", doc.getFileId(), e);
        }
    }
}